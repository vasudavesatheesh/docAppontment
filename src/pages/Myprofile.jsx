...
const Myprofile = () => {
  const[userData,setuserData]=useState({
    ...
    phone:'123 456 789',
    address:{
      line1:'Noway land nowhere ',
      line2:'null street,empty city'
    },
    gender:'Male',
    doB:"2003-10-24"
  })
  ...
  return (
    <div className='max-w-lg flex flex-col  gap-2 text-sm'>
      <img className='w-60 h-30 rounded' src={userData.image} alt="" />
          {
            isEdit
            ?
            <input className='bg-gray-100 text-3xl font-medium max-w-60 mt-4' type="text" name="" id="" value={userData.name} onChange={(e)=>setuserData(prev=>({...prev,name:e.target.value}))} />
            :
            <p className='font-medium text-3xl text-neutral-800 mt-4 '>{userData.name}</p>
          }
          <hr className='bg-zinc-400 h-[ipx] border-none' />
          <div>
